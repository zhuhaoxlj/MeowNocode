    ...
    >
      <div className={`p-4 flex-1 flex flex-col min-w-[240px] relative ${!isRightSidebarPinned && isRightSidebarHovered ? 'h-full overflow-y-auto' : ''}`}>
        ...
        <button
          ...
          className="absolute top-4 left-4 z-10 flex items-center justify-center w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 transition-all duration-300 ease-in-out hover:bg-gray-300 dark:hover:bg-gray-600 hover:scale-110"
          aria-label={isRightSidebarPinned ? "取消固定右侧栏" : "固定右侧栏"}
          title={isRightSidebarPinned ? "取消固定右侧栏" : "固定右侧栏"}
        >
          {isRightSidebarPinned ? (
