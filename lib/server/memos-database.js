  ...
  ensureIndexes() {
    try {
      ...
      this.db.prepare(`
        ...
        ON memo(creator_id, row_status, updated_ts DESC)
      `).run();
      
      // 2. memo 表的 updated_ts 索引（用于排序 - 编辑过的 memo 排在前面）
      this.db.prepare(`
        CREATE INDEX IF NOT EXISTS idx_memo_updated_ts 
        ON memo(updated_ts DESC)
      `).run();
